...
(function () {
  ...
  var points2 = [{"x":1,"y":1}];
  var points3 = [{"x":116,"y":404},{"x":161,"y":617},{"x":16,"y":97}, {"x":430,"y":536}];
  ...
  ctx.strokeStyle = "#f18d00";
  
  
  function addCitiesAndDrawRoute(cities) {
  ...
  function findShortestDistance(cities) {
    var startEndCity = cities.shift();
    var shortestDistance = null;
    var shortestDistanceArray = [];
    for(var i = 0; i < cities.length; i++){
      var currentDistance = calcDistanceBetweenTwoCities(startEndCity, cities[i]);
      var currentDistanceArray = [startEndCity, cities[i]];
      for(var j = 0; j < cities.length; j++){
        if(i !== j){
          currentDistance += calcDistanceBetweenTwoCities(cities[i], cities[j]);
          currentDistanceArray.push(cities[j])
        }
        if(j === cities.length - 1){
          currentDistance += calcDistanceBetweenTwoCities(cities[j], startEndCity);
          currentDistanceArray.push(startEndCity)
        }
      }
      if(currentDistance < shortestDistance || shortestDistance === null){
        shortestDistance = currentDistance;
        shortestDistanceArray = currentDistanceArray
      }
    }
    return {
      shortestDistanceArray: shortestDistanceArray,
      shortestDistance: shortestDistance
    };
  }
  
  function findMostEfficientBandRoute(cities) {
    if(!cities){
      throw new Error('Please enter array of cities cords')
    } else if(cities.length < 2){
      throw new Error('Must have at least two cities to find shortest route')
    }
    var bestRoute = findShortestDistance(cities);
    addCitiesAndDrawRoute(bestRoute.shortestDistanceArray);
    createDiv('The most efficient band route distance is approximately: ' + bestRoute.shortestDistance.toLocaleString())
  }
  
  function createDiv(string, error) {
  ...
  try{
    findMostEfficientBandRoute(points200)
  }catch (e){
    createDiv(e.message, e)
  }
}());
